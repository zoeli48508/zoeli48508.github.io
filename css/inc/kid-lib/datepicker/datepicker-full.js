(function () {
      ...
      Object.assign(this, config, {
        ...
        element: parseHTML(
          `<div class="datepicker-view ${
            picker.datepicker.config.selectWeek ? "is-weekview" : ""
          }"></div>`
        ).firstChild,
        selected: [],
      });
  ...
  function weekAndDay(date, prev) {
    var prefixes = [0, 1, 2, 3, 4, 5];

    return prefixes[Math.floor((date.getDate() + prev) / 7)];
  }
  ...
  class DaysView extends View {
    ...
    render() {
      ...
      if (this.calendarWeeks) {
        ...
        let weeks = weekAndDay(
          new Date(this.last),
          new Date(this.first).getDay()
        );
        dayOfTheWeekOf(this.first, 1, 1);
        const weekWord = ["一", "二", "三", "四", "五", "六"];
        Array.from(this.calendarWeeks.weeks.children).forEach((el, index) => {
          el.textContent =
            index <= weeks
              ? `第${weekWord[index >= weeks ? index - weeks : index]}週`
              : "";
        });
      }
      Array.from(this.grid.children).forEach((el, index) => {
        ...
        if (current < this.first) {
          ...
          if (Math.floor(index / 7) + 1 > weeks + 1) {
        ...
        // debugger;
        if (current === this.focused && !this.selectWeek) {
  ...
  function onClickView(datepicker, ev) {
    ...
    if (isMinView) {
      if (datepicker.config.selectWeek) {
        let dates = [];
        let curr = new Date(Number(target.dataset.date));
        let firstDate = curr.getDate() - curr.getDay(); // First day is the day of the month - the day of the week
        let first = new Date(curr.setDate(firstDate)).valueOf();

        for (let i = 0; i <= 6; i++) {
          let dt = new Date(first + 86400000 * i);
          dates.push(dt.valueOf());
        }
  ...
  function processInputDates(datepicker, inputDates, clear = false) {
    ...
    if ((config.multidate || config.selectWeek) && !clear) {
  ...
  function setDate(datepicker, inputDates, options) {
    ...
    // debugger;
    if (newDates.toString() !== datepicker.dates.toString() || datepicker.config.updateOnSameDate) {
  ...
  class Datepicker {
      ...
      const config = (this.config = Object.assign(
        {
          ...
          selectWeek: options.selectWeek || false,
    ...
    setDate(...args) {
      ...
      let dtes = [];
      if (this.config.selectWeek && inputDates.length === 1) {
        let curr = inputDates[0];
        let firstDate = curr.getDate() - curr.getDay(); // First day is the day of the month - the day of the week
        let first = new Date(curr.setDate(firstDate)).valueOf();

        for (let i = 0; i <= 6; i++) {
          let dt = new Date(first + 86400000 * i);
          dtes.push(dt.valueOf());
        }
      } else {
        dtes = inputDates;
      }

      setDate(this, dtes, opts);
    }
